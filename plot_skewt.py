...
def main():
    ...
    # Convert wind components to km/h for the custom barb logic
    ...
    # CUSTOM BARB LOGIC:
    # ax.barbs draws a half-feather at 5 and a full-feather at 10.
    # By passing (kmh / 2), a 5kmh wind becomes 2.5 (no feather), 
    # and 10kmh becomes 5 (half feather). 
    # To get 5kmh=Half and 10kmh=Full, we pass the raw km/h values 
    # but tell the function to use a 5/10/50 base.
    step = max(1, len(z_plot) // 14) 
    ax2.barbs(np.ones_like(z_plot[::step]) * 45, z_plot[::step], 
              u_plot[::step], v_plot[::step], 
              barb_increments=dict(half=5, full=10, flag=50), # Set custom km/h thresholds
