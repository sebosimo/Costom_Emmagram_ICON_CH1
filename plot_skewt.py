...
def main():
    ...
    # LOOP 2: Thermal Gradients (Orange Dashed 0.5C/100m = 5C/km)
    # These are now disentangled and clearer
    for temp_base in range(-150, 151, 5):
    ...
    # 4. Dry Adiabats (Brown) - 5°C intervals
    ...
    # We use a color scale where:
    # High Lapse Rate (Unstable) = Red
    # Standard Lapse Rate (~6.5) = Yellow/Green
    # Low Lapse Rate / Inversion = Blue
    norm = Normalize(vmin=-2, vmax=11) # Range covering inversions to super-adiabatic
    ...
    # Dewpoint remains solid green for clarity
    ...
    title_str = (f"Payerne | ICON-CH1 Run: {ref_dt.strftime('%Y-%m-%d %H:%M')} UTC\n"
                 f"Temperature Color: Lapse Rate (Red=Unstable, Blue=Inversion)")
