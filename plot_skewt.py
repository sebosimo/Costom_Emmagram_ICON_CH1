...
def main():
    ...
    # Calculate Dewpoint (Td) using Magnus-Tetens
    ...
    def skew_x(temp, press):
        """Transforms Temperature into Skewed X-coordinate"""
        # The factor 'SKEW' determines the 45-degree slant relative to log-P
        return temp + SKEW * np.log10(P_BOT / press)

    # --- 4. FIGURE SETUP ---
    fig = plt.figure(figsize=(12, 16))
    ax1 = fig.add_axes([0.1, 0.1, 0.65, 0.8]) # Skew-T Panel
    ax2 = fig.add_axes([0.75, 0.1, 0.15, 0.8]) # Wind Panel (No gap)

    P_TOP = 400
    T_MIN, T_MAX = -60, 40 # Temperature range at surface

    # --- 5. BACKGROUND GRID (Physics Lines) ---
    z_ref = np.linspace(0, 8000, 100)
    p_ref = 1013.25 * (1 - 2.25577e-5 * z_ref)**5.25588

    # A. Slanted Isotherms (45° to the right)
    for iso_t in range(-80, 81, 10):
        ax1.plot(skew_x(iso_t, p_ref), p_ref, color='black', alpha=0.15, lw=0.7)

    # B. Dry Adiabats (1°C / 100m - also skewed)
    ...
    # C. Mixing Ratio (g/kg - also skewed)
    ...
    print("Success: 45-degree skewed emagram generated.")
