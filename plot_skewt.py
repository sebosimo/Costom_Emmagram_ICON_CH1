...
def main():
    ...
    # --- Transformation Logic for 45° Isotherms ---
    ax1 = fig.add_axes([0.1, 0.1, 0.55, 0.8])
    
    # Skew transformation: tilts the X-axis so vertical lines on the plot
    # represent the 45-degree slanted isotherms relative to linear Y (km)
    skew_angle = 45 
    base_trans = ax1.transData
    skew = transforms.Affine2D().skew_deg(skew_angle, 0)
    t_skew = skew + base_trans

    # --- PANEL 1: CUSTOM SKEWED THERMO DIAGRAM ---
    ax1.set_ylim(0, 8)   # Linear Altitude in km
    ax1.set_xlim(-25, 45) # Temperature range in Celsius
    
    # Draw Isotherms (Now manually using plot with skew transform)
    for temp in range(-50, 61, 10):
        ax1.plot([temp, temp], [0, 8], color='blue', alpha=0.1, linestyle='--', transform=t_skew, zorder=1)

    # Helper Lines: Pressure levels for calculations
    ...
    for theta in range(-20, 120, 10):
        ...
        ax1.plot(t_adiabat, z_range.m, color='brown', alpha=0.15, linewidth=1, transform=t_skew, zorder=2)

    # Moist Adiabats (Equivalent Potential Temperature)
    for t_start in range(-20, 41, 5):
        t_moist = mpcalc.moist_lapse(p_range, t_start * units.degC, 1000 * units.hPa).to(units.degC).m
        ax1.plot(t_moist, z_range.m, color='blue', alpha=0.1, linewidth=0.8, transform=t_skew, zorder=2)

    # Mixing Ratio Lines (FIXED: vapor_pressure -> dewpoint)
    for w in [1, 2, 4, 7, 10, 16, 24, 32]:
        w_val = w * units('g/kg')
        # Correct path: mixing ratio -> vapor pressure -> dewpoint
        e_w = mpcalc.vapor_pressure(p_range, w_val)
    ...
    ax2.set_yticklabels([]) # Hide labels to align with ax1
    ax2.grid(True, alpha=0.2)

    # 4. Final Polish
    ref_time_str = datetime.datetime.fromisoformat(ds.attrs["ref_time"]).strftime('%Y-%m-%d %H:%M')
    fig.suptitle(f"Linear Altitude Sounding | Payerne | {ref_time_str} UTC", fontsize=16)

    plt.savefig("latest_skewt.png", dpi=150, bbox_inches='tight')
    print("Success: Generated linear altitude diagram with 45° skewed isotherms and moist adiabats.")
