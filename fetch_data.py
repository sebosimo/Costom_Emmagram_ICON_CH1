...
HORIZON = "P0DT0H" # Change this if you want a forecast (e.g., P0DT3H)
...
def main():
    ...
    # ICON-CH1 updates every 3 hours
    ...
    for ref_time in times_to_try:
        ...
        if os.path.exists(cache_path) and not force:
            print(f">>> Run {time_tag} found in cache.")
            return
        ...
        try:
            ...
            for var in CORE_VARS:
                req = ogd_api.Request(collection="ogd-forecasting-icon-ch1", variable=var,
                                     reference_datetime=ref_time, horizon=HORIZON, perturbed=False)
            ...
            h_delta = int(HORIZON.split('T')[1].replace('H', ''))
            valid_time = ref_time + datetime.timedelta(hours=h_delta)
            ...
            ds.attrs = {
                ...
                "lead_time": str(h_delta)
            }
            
            # Clean metadata to prevent NetCDF errors
            for v in ds.data_vars: ds[v].attrs = {}
            for c in ds.coords: ds[c].attrs = {}

            ds.to_netcdf(cache_path)
            print(f">>> SUCCESS: Saved {time_tag}")
            return 
